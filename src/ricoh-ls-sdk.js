 ...
 * @property {boolean} in_recording
 * /

/**
 * @typedef {object} RoomStatus
 * @property {RecordingStatus} recording_status
 * /
 ...
 * @property {RoomStatus} room_status
 */
...
/**
 * @typedef {object} NotifySFURecordingUpdateMessage
 * @property {"notify.sfu.recording.update"} message_type
 * @property {RecordingStatus} recording_status
 */
...
/** @type {string} */
const CLIENT_JSON_URL = "https://signaling.livestreaming.mw.smart-integration.ricoh.com/v1/sdk/data/client.json";
...
class LSTrack {
  ...
  constructor(mediaStreamTrack, stream, option) {
    ...
    /**
     * @package
     * @type {{mediaStreamTrack: MediaStreamTrack|null, mid: string}}
     */
    this.storedMid = { mediaStreamTrack: null, mid: "" };
...
class Client extends EventTarget {
  constructor() {
     ...
     */
    this.parameterErrorTable = new Map();
  ...
  /**
   * LSTrack(local_track) に対するmidを取得する
   *
   * @public
   * @param {LSTrack} [lsTrack]
   * @param {ConnectionID} [connection_id]
   * @returns {string}
   */
  getLocalTrackMid(lsTrack, connection_id) {
    this.putLog("debug", "API Call: getLocalTrackMid");
    if (!lsTrack) throw new SDKError(new ErrorData(45026, "InvalidTrackOnGetLocalTrackMid"));

    return this.withErr(61054, () => {
      if (lsTrack.storedMid.mediaStreamTrack === lsTrack.mediaStreamTrack && lsTrack.storedMid.mid !== "") return lsTrack.storedMid.mid;

      let transceivers = [];
      if (connection_id === "sfu" && this.isSfuRoom()) {
        transceivers = transceivers.concat(this.sfupc?.getTransceivers());
      }
      this.peers.forEach((peer) => {
        if (peer.connection_id === connection_id) transceivers = transceivers.concat(peer.getTransceivers());
      });

      for (const transceiver of transceivers) {
        if (transceiver.sender?.track?.id === lsTrack.mediaStreamTrack.id) {
          lsTrack.storedMid = { mediaStreamTrack: lsTrack.mediaStreamTrack, mid: transceiver.mid };
  ...
  connectMain(client_id, access_token, option) {
    this.withErr(
      ...
      () => {
        ...
        this.ws.on("notify.sfu.recording.update", this.onNotifySFURecordingUpdate.bind(this));
  ...
  connect(client_id, access_token, option) {
    ...
    fetch(CLIENT_JSON_URL)
      .catch((e) => {
        this.putLog("debug", `Fetch Exception: ${e}`);
        throw new SDKError(new ErrorData(54003, "PreConnectError", e));
      })
      .then((res) => {
        if (!res.ok) {
          this.putLog("debug", `Fetch HTTP Error: ${res}`);
          throw new SDKError(new ErrorData(54003, "PreConnectError", res));
        } else return res.json();
      })
      .then((json) => {
        this.parameterErrorTable = json.parameterError;
        this.signalingErrorTable = json.signalingError;
        if (Object.keys(this.parameterErrorTable).length === 0 || Object.keys(this.signalingErrorTable).length === 0) {
          throw new Error("Data API file broken");
        }
        return true;
      })
      .catch((e) => {
        if (e instanceof SDKError) this.emitError(e.errdata.getDetail().code, e.errdata.getDetail().error, e);
        else this.internalError(61052, e);
        if (this.state === "idle" || this.state === "connecting" || this.state === "open") this.setState("closing", "closing");
        this.setState("closed", "close");
        return false;
      })
      .then((success) => {
        if (success) this.connectMain(client_id, access_token, option);
      });
  }
   ...
   * @param {LocalLSTrack} localLSTrack
   */
  async changeMuteMain(nextMuteType, localLSTrack) {
    await this.withErrAsync(61040, async () => {
      await this.changeMuteState(nextMuteType, localLSTrack);
      if (this.isSfuRoom()) {
  ...
  async changeMute(lsTrack, nextMuteType) {
    ...
    if (localLSTrack) {
      ...
      if (curMuteType !== nextMuteType) await this.changeMuteMain(nextMuteType, localLSTrack);
    }
  }
   ...
   * @returns {{nextMediaStreamTrack: MediaStreamTrack|null, trackEnabled: boolean}}
   */
  nextTrackOnChangeMute(nextMuteType, curMediaStreamTrack) {
   ...
   * @returns {Promise<void>}
   */
  async changeMuteState(nextMuteType, localLSTrack) {
    const { nextMediaStreamTrack, trackEnabled } = this.nextTrackOnChangeMute(nextMuteType, localLSTrack.mediaStreamTrack);
  ...
  onTrackP2P(peer, e) {
    ...
    this.withErr(
      ...
      () => {
        ...
        let mid = "";
        if (e.transceiver.mid) mid = e.transceiver.mid;

        obj = { connection_id: peer.connection_id, mediaStreamTrack: track, stream, mid };
  ...
  async onTrackSFU(peer, e) {
    ...
    this.withErr(
      ...
      () => {
        ...
        obj = { connection_id, mediaStreamTrack: track, stream, meta: metadata, mute, mid };
  ...
  onWSClose(e) {
    ...
    if (error === "paramerr") {
      ...
      if (paramerr !== undefined) this.emitError(40000 + e.code, paramerr, e);
  ...
  onConnectSuccess(e) {
    ...
    this.withErr(
      ...
      () => {
        ...
        const { ice_servers, room_session_id, room_status } = e;
        ...
        recording = room_status?.recording_status?.in_recording;
      },
      () => {
        ...
        if (recording) this.emit("updaterecording", { in_recording: true });
      },
    );
  }
  ...
  async onP2PRequestOffer(e) {
    ...
    await this.withErrAsync(61016, async () => {
      ...
      for (const v of this.initialReplaceMediaStreamTrack) {
        await this.replaceLSTrack(v.mediaStreamTrack, v.localLSTrack);
      }
      for (const localLSTrack of this.localLSTracks) {
        await this.changeMuteState(localLSTrack.muteType, localLSTrack);
      }
   ...
   * @param {Peer} peer
   */
  async setEarlyCandidate(peer_connection_id, peer) {
  ...
  async onP2PRelayOffer(e) {
    ...
    await this.withErrAsync(61017, async () => {
      ...
      await this.setEarlyCandidate(peer_connection_id, peer);
  ...
  /**
   * @private
   * @param {NotifySFURecordingUpdateMessage} e
   */
  onNotifySFURecordingUpdate(e) {
    let recording = false;
    this.withErr(
      61053,
      () => {
        const { recording_status } = e;
        recording = recording_status?.in_recording;
      },
      () => {
        this.emit("updaterecording", { in_recording: recording });
      },
    );
  }
  ...
  onError(e) {
    ...
    if (sigerr !== undefined) {
      if (sigerr !== "") this.emitError(45000 + error_code, sigerr, e);
   ...
   */
  getParamError(code) {
    return this.parameterErrorTable[code.toString()];
  }
  ...
  getSignalingError(code) {
    return this.signalingErrorTable[code.toString()];
  }
