...
class LocalLSTrack {
  ...
  changeFramerate(framerate) {
    ...
    if (this.senders.every((sender) => sender.track === null)) this.reservedFramerate = framerate;
  }

  updateEncodingParameters() {
    if (this.reservedFramerate !== null) {
      this.changeFramerate(this.reservedFramerate);
      this.reservedFramerate = null;
    }
  }
}
...
class Client extends ET {
  ...
  changeVideoSendFramerate(framerate) {
    ...
    if (this.state !== "open") throw new SDKError(new ErrorData(45016, "BadStateOnChangeVideoSendFramerate"));
    if (framerate < 0.0 || 10000.0 < framerate) throw new SDKError(new ErrorData(45017, "InvalidMaxFramerateOnChangeVideoSendramerate"));

    try {
      this.reservedFramerate = framerate;
      this.localLSTracks.forEach((loclsTrack) => {
        loclsTrack.changeFramerate(framerate);
      });
    } catch (e) {
      this.internalError(61050, e);
    }
  }
  ...
  changeVideoSendBitrate(maxBitrateKbps) {
    ...
    if (!this.sfupc) throw new SDKError(new ErrorData(45018, "UnsupportedRoomSpecTypeOnChangeVideoSendBitrate"));
  ...
  onWSOpen() {
    try {
      ...
      const connectMessage = {
        ...
        sdk_info: { platform: "web", version: "1.4.0+20220420" },
  ...
  async onP2PRequestOffer(e) {
    ...
    try {
      ...
      this.localLSTracks.forEach(async (localLSTrack) => {
        if (this.reservedFramerate) localLSTrack.changeFramerate(this.reservedFramerate);
      });
    } catch (e) {
